...
#define POD_CACHE_ALIGNED
    ...
    eva_t A_device, lxx_device,lyy_device,lxy_device,gray_device,cim_device,Output_device;
    BSG_CUDA_CALL(hb_mc_device_malloc(&device, size * sizeof(short)*3, &A_device));
    BSG_CUDA_CALL(hb_mc_device_malloc(&device, size * sizeof(short), &gray_device));
    BSG_CUDA_CALL(hb_mc_device_malloc(&device, size * sizeof(short), &lxx_device));
    BSG_CUDA_CALL(hb_mc_device_malloc(&device, size * sizeof(short), &lyy_device));
    BSG_CUDA_CALL(hb_mc_device_malloc(&device, size * sizeof(short), &lxy_device));
    BSG_CUDA_CALL(hb_mc_device_malloc(&device, size * sizeof(short), &cim_device));
    BSG_CUDA_CALL(hb_mc_device_malloc(&device, size * sizeof(short), &Output_device));

    printf("A_device Addr: %x\n", A_device);
    printf("gray_device Addr: %x\n", gray_device);
    printf("lxx_device Addr: %x\n", lxx_device);
    printf("lyy_device Addr: %x\n", lyy_device);
    printf("lxy_device Addr: %x\n", lxy_device);
    printf("cim_device Addr: %x\n", cim_device);
    ...
    #define CUDA_ARGC 9
    uint32_t cuda_argv[CUDA_ARGC] = {A_device,gray_device,lxx_device,lyy_device,lxy_device,cim_device,Output_device,image_size_x,image_size_y};
    ...
    BSG_CUDA_CALL(hb_mc_device_dma_to_host(&device, &dtoh_job, 6));
    ...
    createBMP("/local1/jwang710/bsg_bladerunner/bsg_replicant/examples/hb_hammerbench/apps/harris_color/tile-x_8__tile-y_4__image-size-x_64__image-size-y_64__warm-cache_yes/gray_output.bmp", image_size_x, image_size_y,gray);
    createBMP("/local1/jwang710/bsg_bladerunner/bsg_replicant/examples/hb_hammerbench/apps/harris_color/tile-x_8__tile-y_4__image-size-x_64__image-size-y_64__warm-cache_yes/lxx_output.bmp", image_size_x, image_size_y,lxx_d);
    createBMP("/local1/jwang710/bsg_bladerunner/bsg_replicant/examples/hb_hammerbench/apps/harris_color/tile-x_8__tile-y_4__image-size-x_64__image-size-y_64__warm-cache_yes/lxx_expected.bmp", image_size_x, image_size_y,lxx);
    createBMP("/local1/jwang710/bsg_bladerunner/bsg_replicant/examples/hb_hammerbench/apps/harris_color/tile-x_8__tile-y_4__image-size-x_64__image-size-y_64__warm-cache_yes/cim_output.bmp", image_size_x, image_size_y,cim_d);
    createBMP("/local1/jwang710/bsg_bladerunner/bsg_replicant/examples/hb_hammerbench/apps/harris_color/tile-x_8__tile-y_4__image-size-x_64__image-size-y_64__warm-cache_yes/cim_expected.bmp", image_size_x, image_size_y,cim);
    // Validate
    //gray
    for (int i = 0; i < image_size_x*image_size_y; i++) {
      if (gray_expected[i] != gray[i]) {
        printf("GRAY FAIL [%d]: expected=%d, actual=%d\n", i, gray_expected[i], gray[i]);
    ...
    // Validate
    //lxx
    for (int i = 1; i < image_size_x-1; i++) {
      for (int j = 1; j < image_size_y-1; j++) {
        int index=i+j*image_size_x;
        if (lxx_d[index] != lxx[index]) {
            printf("LXX FAIL [%d]: expected=%d, actual=%d\n", index, lxx[index], lxx_d[index]);
            BSG_CUDA_CALL(hb_mc_device_finish(&device));
            return HB_MC_FAIL;
        }
      }
    ...
    for (int i = 1; i < image_size_x-1; i++) {
      for (int j = 1; j < image_size_y-1; j++) {
        int index=i+j*image_size_x;
        if (lyy_d[index] != lyy[index]) {
          printf("LYY FAIL [%d]: expected=%d, actual=%d\n", i, lyy[i], lyy_d[i]);
          BSG_CUDA_CALL(hb_mc_device_finish(&device));
          return HB_MC_FAIL;
        }
      }
    ...
    for (int i = 1; i < image_size_x-1; i++) {
      for (int j = 1; j < image_size_y-1; j++) {
        int index=i+j*image_size_x;
        if (lxy_d[index] != lxy[index]) {
          printf("LXY FAIL [%d]: expected=%d, actual=%d\n", i, lxy[i], lxy_d[i]);
          BSG_CUDA_CALL(hb_mc_device_finish(&device));
          return HB_MC_FAIL;
        }
      }
    ...
    for (int i = 2; i < image_size_x-2; i++) {
      for (int j = 2; j < image_size_y-2; j++) {
        int index=i+j*image_size_x;
        if (cim[index] != cim_d[index]) {
          printf("CIM FAIL [%d]: expected=%d, actual=%d\n", index, cim[index], cim_d[index]);
          BSG_CUDA_CALL(hb_mc_device_finish(&device));
          //printImage(cim, image_size_x, image_size_y);
          //printImage(cim_d, image_size_x, image_size_y);
          return HB_MC_FAIL;
        }
      }
    }
