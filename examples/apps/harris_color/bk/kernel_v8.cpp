...
kernel_harris_color(short * A, short *G,short *lxx,short *lyy,short *lxy,short *cim,short *Output, int N_X, int N_Y) {
  ...
  for(int y=0;y<y_tile_len;y++){
    for(int x=1;x<x_tile_len-1;x++){
      ...
      int g_top_left_index=(x-1)+(y+init_index_y-1)*N_X;
      int g_top_index=(x)+(y+init_index_y-1)*N_X;
      int g_top_right_index=(x+1)+(y+init_index_y-1)*N_X;
      int g_left_index=(x-1)+(y+init_index_y)*N_X;
      //int middle_index=(x)+(y+1)*N_X;
      int g_right_index=(x+1)+(y+init_index_y)*N_X;
  ...
  for(int y=0;y<y_tile_len;y++){
    for(int x=2;x<x_tile_len-2;x++){
      ...
      int g_top_right_index=(x+1)+(y+init_index_y-1)*N_X;
      int g_left_index=(x-1)+(y+init_index_y)*N_X;
      int g_middle_index=(x)+(y+init_index_y)*N_X;
      int g_right_index=(x+1)+(y+init_index_y)*N_X;
      int g_bot_left_index=(x-1)+(y+init_index_y+1)*N_X;
      int g_bot_index=x+(y+init_index_y+1)*N_X;
      int g_bot_right_index=(x+1)+(y+init_index_y+1)*N_X;
  ...
  //write_output_backduid
  //for(int y=0;y<tile_image_size_y-4;y++){
  //  for(int x=0;x<x_tile_len-4;x++){
  //    int local_inline_index=(x+2)+(y+2)*x_tile_len;
  //    int global_x=x+init_index_x;
  //    int global_y=y+init_index_y;
  //    int global_index=global_x+global_y*N_X;
