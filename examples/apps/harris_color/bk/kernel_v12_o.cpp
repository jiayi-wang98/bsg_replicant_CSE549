...
kernel_harris_color(short * A,short *Output, int N_X, int N_Y) {
  ...
  for (int y = 0; y < tile_image_size_y; y++) {
    ...
    //x=0~2
    if((global_y<0)||(global_y>=N_Y)) continue;
    //left column no 0-2
    if(init_index_x!=0){
      ...
      gray[local_index]=(int) ((77 *myA[0] + 150 * myA[1]+ 29 * myA[2]) >> 8);
      gray[local_index+1]=(int) ((77 *myA[3] + 150 * myA[4]+ 29 * myA[5]) >> 8);
      gray[local_index+2]=(int) ((77 *myA[6] + 150 * myA[7]+ 29 * myA[8]) >> 8);
    }
    ...
    gray[local_index+1]=(int) ((77 *myA[3] + 150 * myA[4]+ 29 * myA[5]) >> 8);
    gray[local_index+2]=(int) ((77 *myA[6] + 150 * myA[7]+ 29 * myA[8]) >> 8);
    gray[local_index+3]=(int) ((77 *myA[9] + 150 * myA[10]+ 29 * myA[11]) >> 8);
    ...
    if(init_index_x!=N_X-x_tile_len){
      global_index+=4;
      local_index+=4;
      myA[0]=A[global_index*3];
      myA[1]=A[global_index*3+1];
...
//test end
  ...
  //      int bot_index=(x)+(y+1)*tile_image_size_x;
  //      int bot_right_index=(x+1)+(y+1)*tile_image_size_x;
  //      cim_reg[0]=cim_local[top_left_index];
  //      cim_reg[1]=cim_local[top_index];
  //      cim_reg[2]=cim_local[top_right_index];
  //      cim_reg[3]=cim_local[left_index];
  //      is_max= (current_cim >= cim_reg[0] && current_cim >= cim_reg[1]) &&(current_cim >= cim_reg[2] && current_cim >= cim_reg[3]);
  //      cim_reg[5]=cim_local[right_index];
  //      cim_reg[6]=cim_local[bot_left_index];
  //      cim_reg[7]=cim_local[bot_index];
  //      cim_reg[8]=cim_local[bot_right_index];
  //      is_max = is_max &&(current_cim >= cim_reg[5] && current_cim >= cim_reg[6])&&(current_cim >= cim_reg[7] && current_cim >= cim_reg[8]);
